...
const firebaseConfig = {
  apiKey: "AIzaSyCfLBi8RFp8zrFfZP_8G6Nqsqtuy2YXL6o",
  authDomain: "sucatasaojosesite.firebaseapp.com",
  databaseURL: "https://sucatasaojosesite-default-rtdb.firebaseio.com/",
  projectId: "sucatasaojosesite",
  storageBucket: "sucatasaojosesite.firebasestorage.app",
  messagingSenderId: "908319944936",
  appId: "1:908319944936:web:99a8c0bb377e12e508995d"
};
...
const MARKETING_TRIGGERS = [
  ...
  { id: "buying", label: "♻️ Compramos", badgeClass: "deal" }
];
...
function priceLabel(p) {
  if (p && (p.negotiable === true || p.price === null || p.price === undefined || p.price === "")) {
...
function isNegotiable(p) {
  return !!(p && (p.negotiable === true || p.price === null || p.price === undefined || p.price === ""));
}
...
function applyTheme(theme) {
  ...
  if (btn) {
    ...
    btn.title = state.theme === "light" ? "Mudar para tema escuro" : "Mudar para tema claro";
  }
  try { localStorage.setItem("ssj-theme", state.theme); } catch (e) {}
}
...
function getFilteredProdutos() {
  ...
  return state.produtos.filter((p) => {
    const haystack = normalize([p.title, p.description, p.cities, isNegotiable(p) ? "preco a negociar negociar" : ""].join(" "));
...
function openProdutoModal(id) {
  ...
  priceEl.textContent = priceLabel(p);
  priceEl.classList.toggle("price-negotiable", isNegotiable(p));
  const buyBtn = document.getElementById("modal-buy");
  buyBtn.href = p.buyLink || "https://wa.me/5588988470190";
  buyBtn.textContent = isNegotiable(p) ? "Negociar preço" : "Comprar agora";
...
function syncNegotiableField() {
  const check = document.getElementById("produto-negociar");
  const priceInput = document.getElementById("produto-preco");
  if (!check || !priceInput) return;
  priceInput.disabled = check.checked;
  if (check.checked) priceInput.value = "";
  priceInput.placeholder = check.checked ? "Preço a negociar" : "Ex: 12.50";
}
...
async function saveProduto(e) {
  ...
  const negotiable = document.getElementById("produto-negociar").checked;
  const rawPrice = document.getElementById("produto-preco").value.replace(",", ".");
  const price = negotiable || rawPrice === "" ? null : parseFloat(rawPrice);
  ...
  if (!negotiable && (price === null || isNaN(price))) {
    showToast('Informe o preço ou marque "Preço a negociar".', "error");
    return;
  }

  const data = { title, description, cities, price: negotiable ? null : price, negotiable, buyLink, image, triggers, order };
...
function editProduto(id) {
  ...
  document.getElementById("produto-preco").value = isNegotiable(p) ? "" : (p.price ?? "");
  document.getElementById("produto-negociar").checked = isNegotiable(p);
  syncNegotiableField();
...
function resetProdutoForm() {
  ...
  document.getElementById("produto-negociar").checked = false;
  syncNegotiableField();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const negociarCheck = document.getElementById("produto-negociar");
  if (negociarCheck) negociarCheck.addEventListener("change", syncNegotiableField);
